...
- Follows the file conventions in `CLAUDE.md` (slugs, frontmatter, wikilinks both directions).
...
This is in addition to the top-level `CONTRIBUTORS.md` file at the repository root. The roll lists everyone with at least one merged PR; the per-page line is for substantive shaping of that specific page.
...
`status: draft`. The charter itself is one of the [[05_open-questions]] resolved in this pass. It will mutate as real contributions accumulate and the actual failure modes become visible. Material revisions are logged in `wiki/log.md`.
...
- `CONTRIBUTORS.md` — the merged-contributor roll.
