...
- **Karpathy vs. Sutskever — Iron Man vs straight-shot.** [[sutskever-age-of-research]] articulates the *opposite* deployment posture: build superintelligence in a silo (the "straight-shot" method) and release only when ready. Karpathy's slider treats partial autonomy as the durable design pattern; Sutskever treats partial autonomy as the slow, market-iterative path he is willing to abandon when the underlying capability is sufficient. The interviewer's pushback — complex systems (aviation, Linux) became robust because they were deployed, failed, and were iteratively patched — is essentially the Iron Man argument applied to civilisational deployment. Sutskever appears to be updating; the tension is unresolved. The slider is also implicitly anchored by the [[eval-real-world-gap]]: as long as benchmarks and real performance diverge, the slider cannot be pushed far right safely.
...
- [[observed-exposure-measure]] — the automated-vs-augmentative weighting in this measure operationalises the slider distinction at usage-data level.
